...
| `0xFFFC - 0xFFFD` | Syscall handler (under System Call Exception) |
...
A basic synchronous exception system is supported with the System Call Extension. This extension
is not a requirement for a full implementation of the core architecture.
...
```
CAUSE = RS << 8 | 0x01
if (E == 0) { exit(RS) } else { PC = Memory[0xFFFD] << 8 | Memory[0xFFFC] }
```
...
code, stored in the `CAUSE` special register. The syscall handler address at `0xFFFC - 0xFFFD` is
