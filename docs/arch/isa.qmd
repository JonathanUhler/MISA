...
The System Call Extention extends the `HALT` instruction in the core architecture to support basic
synchronous exceptions that may be used to enable system calls or other similar features.
...
Meaning: `if (H) { exit(RS) } else { MARK }`
...
Extension by setting the `H` bit to `0`. In that case, the processor must adhere to the core
