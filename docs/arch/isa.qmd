...
| `RW`     | Saved                     | `RWX`         | 1011     |
| `RX`     | Saved                     |               | 1100     |
| `RY`     | Assembler scratch         | `RYZ`         | 1101     |
| `RZ`     | Assembler scratch         |               | 1110     |
| `RT`     | Argument/return/temporary | N/A           | 1111     |
...
loading an address to jump to). As such, the value of `RYZ` should not be assumed to persist over
time.
...
the `FLAGS` register are: `ADD`, `ADC`, `SUB`, `SBB`. Instructions which may, depending on the
implementation, update the `FLAGS` register are: `HALT`, `AND`, `OR`, `XOR`, `RRC`, `SET`.
