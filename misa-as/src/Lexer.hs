...
lexerGetOpcode cs =
  case lexerGetIdentifier cs of
    Just (identifier, rest) -> case identifier of
      ...
      -- Pseudo instructions
      IdentifierToken "NOT"   -> Just (OpcodeToken NOT, rest)
      IdentifierToken "MOV"   -> Just (OpcodeToken MOV, rest)
      IdentifierToken "LDI"   -> Just (OpcodeToken LDI, rest)
      IdentifierToken "CALL"  -> Just (OpcodeToken CALL, rest)
      IdentifierToken "ENTRY" -> Just (OpcodeToken ENTRY, rest)
      IdentifierToken "RET"   -> Just (OpcodeToken RET, rest)
      IdentifierToken "PUSH"  -> Just (OpcodeToken PUSH, rest)
      IdentifierToken "POP"   -> Just (OpcodeToken POP, rest)
      _                       -> Nothing
    Nothing -> Nothing
...
lexerGetRegister cs =
  case lexerGetIdentifier cs of
    Just (identifier, rest) -> case identifier of
      ...
      IdentifierToken "ZERO"    -> Just (RegisterToken R0,  rest)
      IdentifierToken "R1"      -> Just (RegisterToken R1,  rest)
      IdentifierToken "RALO"    -> Just (RegisterToken R1,  rest)
      IdentifierToken "R2"      -> Just (RegisterToken R2,  rest)
      IdentifierToken "RAHI"    -> Just (RegisterToken R2,  rest)
      IdentifierToken "R3"      -> Just (RegisterToken R3,  rest)
      IdentifierToken "SPLO"    -> Just (RegisterToken R3,  rest)
      IdentifierToken "R4"      -> Just (RegisterToken R4,  rest)
      IdentifierToken "SPHI"    -> Just (RegisterToken R4,  rest)
      IdentifierToken "R5"      -> Just (RegisterToken R5,  rest)
      IdentifierToken "A0"      -> Just (RegisterToken R5,  rest)
      IdentifierToken "R6"      -> Just (RegisterToken R6,  rest)
      IdentifierToken "A1"      -> Just (RegisterToken R6,  rest)
      IdentifierToken "R7"      -> Just (RegisterToken R7,  rest)
      IdentifierToken "A2"      -> Just (RegisterToken R7,  rest)
      IdentifierToken "R8"      -> Just (RegisterToken R8,  rest)
      IdentifierToken "A3"      -> Just (RegisterToken R8,  rest)
      IdentifierToken "R9"      -> Just (RegisterToken R9,  rest)
      IdentifierToken "S0"      -> Just (RegisterToken R9,  rest)
      IdentifierToken "R10"     -> Just (RegisterToken R10, rest)
      IdentifierToken "S1"      -> Just (RegisterToken R10, rest)
      IdentifierToken "R11"     -> Just (RegisterToken R11, rest)
      IdentifierToken "T0"      -> Just (RegisterToken R11, rest)
      IdentifierToken "R12"     -> Just (RegisterToken R12, rest)
      IdentifierToken "T1"      -> Just (RegisterToken R12, rest)
      IdentifierToken "R13"     -> Just (RegisterToken R13, rest)
      IdentifierToken "T2"      -> Just (RegisterToken R13, rest)
      IdentifierToken "R14"     -> Just (RegisterToken R14, rest)
      IdentifierToken "T3"      -> Just (RegisterToken R14, rest)
      IdentifierToken "R15"     -> Just (RegisterToken R15, rest)
      IdentifierToken "SCRATCH" -> Just (RegisterToken R15, rest)
      _                         -> Nothing
    Nothing -> Nothing
